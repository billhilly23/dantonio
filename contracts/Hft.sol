...
contract HFT {
  ...
  // Event emitted when an HFT trade is executed
  event HFTTradeExecuted(address indexed market, uint256 amount, uint256 profit);
  ...
  // Function to execute an HFT trade
  function hft() public {
    ...
    // Execute the HFT trade
    executeHFTTrade(market, amount, profit);

    // Emit the HFTTradeExecuted event
    emit HFTTradeExecuted(market, amount, profit);
  }
  ...
  // Function to execute an HFT trade
  function executeHFTTrade(address _market, uint256 _amount, uint256 _profit) internal {
